...
public String userOrderHistory(
    ...
    try {
        ...
        if (date != null && !date.isEmpty()) {
            ...
            LocalDateTime endDate = LocalDate.parse(date).atTime(23, 59, 59);
            ordersPage = orderRepository.findByCreatedAtBetweenOrderByCreatedAtDesc(startDate, endDate, pageRequest);
        } else if (customerName != null && !customerName.trim().isEmpty()) {
            // Xử lý lọc theo tên khách hàng
            ordersPage = orderRepository.findByCustomerNameContainingOrderByCreatedAtDesc(customerName.trim(), pageRequest);
        } else if (cashier != null && !cashier.trim().isEmpty()) {
            // Xử lý lọc theo thu ngân
            ordersPage = orderRepository.findByCreatedByContainingOrderByCreatedAtDesc(cashier.trim(), pageRequest);
        } else {
            // Không có điều kiện lọc, lấy tất cả đơn hàng
            ordersPage = orderRepository.findAllByOrderByCreatedAtDesc(pageRequest);
        }
    ...
    // Endpoint để lấy chi tiết đơn hàng cho form đổi/trả
        ...
        try {
            ...
            if (order.getStatus() != Order.OrderStatus.COMPLETED) {
                return ResponseEntity.badRequest()
                    .body(Map.of("error", "Chỉ những đơn hàng đã hoàn thành mới có thể đổi/trả"));
            }
    ...
    // Endpoint để lấy danh sách sản phẩm có sẵn cho đổi hàng
...
public String viewOrderDetailLegacy(@RequestParam("id") Long id, Model model, HttpSession session) {
    ...
    public ResponseEntity<?> createReturnRequest(@RequestBody Map<String, Object> request) {
        try {
            ReturnExchangeRequest savedRequest = returnExchangeService.createRequest(request);
    ...
    @GetMapping("/returns/status/{orderId}")
    @ResponseBody
    public ResponseEntity<?> getReturnRequestStatus(@PathVariable Long orderId) {
        try {
            Optional<ReturnExchangeRequest> request = returnExchangeService.findByOrderId(orderId);
            ...
            return ResponseEntity.status(500)
                    .body(Map.of("error", "Lỗi khi lấy trạng thái yêu cầu: " + e.getMessage()));
        }
    }
