...
public class OrderService {
    ...
    public Page<Order> findByCustomerId(Long customerId, Pageable pageable) {
        ...
        List<Order> orders = orderRepository.findCustomerOrderHistory(customerId);
        final int start = (int)pageable.getOffset();
        final int end = Math.min((start + pageable.getPageSize()), orders.size());
        Page<Order> result = new org.springframework.data.domain.PageImpl<>(
            orders.subList(start, end), pageable, orders.size());
...
public double getTotalRevenue() {
    ...
    // Get orders by customer ID using order history
    ...
    // Method to get all orders without pagination
...
// ...existing code...
    ...
    public Page<Order> findOrders(
        ...
        if (customerName != null && !customerName.trim().isEmpty()) {
            return orderRepository.findByCustomerNameContaining(customerName.trim(), pageable);
        }
        
        if (orderNumber != null && !orderNumber.trim().isEmpty()) {
            return orderRepository.findByOrderNumberContaining(orderNumber.trim(), pageable);
        }
    ...
    public Page<Order> findByStatus(Order.OrderStatus status, Pageable pageable) {
        return orderRepository.findByStatus(status.toString(), pageable);
    }

    // Get orders by status without pagination
    public List<Order> getOrdersByStatus(Order.OrderStatus status) {
        return orderRepository.findAllByStatus(status.toString());
    }
